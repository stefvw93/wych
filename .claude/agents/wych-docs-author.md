...
- A feature is declared with `define({ props, state, action, output })` and built with `Feature.create({ initialState, reducer, render, subscriptions })`. `subscriptions` is optional. Props and state are `Schema.Struct`s. Messages are `Action("Tag", fields)` / `Action.output("Tag", fields)`, with `fields` optional (`Action("Reverted")`), or several at once as a record keyed by tag: `Action({ Typed: { query: Schema.String }, Cleared: {} })`, `Action.output({ Saved: { id: Schema.String } })`. The `action` and `output` slots take a message, a record, a `Task`, or an array of those: `action: [actions, search]`, `output: outputs`. There is no vocabulary wrapper; `Action.of` does not exist, never document it. `action` takes internal members only, `output` outbound ones.
...
- Async work goes through `Task(name, { success, failure, onError, mode, run })`, which declares `${Name}Resolved` and `${Name}Rejected` actions plus the command. `onError` is optional without `failure` and defaults to `Task.errorMessage`; with a `failure` schema it is required. Samples omit `onError` unless they declare a `failure`. The operation goes into the `action` slot itself (`action: [Clicked, search]`), never `...search.actions`. The field is `TaskValue` with four cases (`Idle | Pending | Resolved | Rejected`), declared with the operation's own `search.schema` (`Task.schema(success)` only for a field no operation owns), written with `Task.start`, read with the total `Task.match`. `...search.into("results")` writes both settle handlers; to do more on a result, write the entry as `SearchResolved: search.resolvedInto("results", (value, { draft }) => { …; return draft; })` (or `rejectedInto`): the field is already written into the draft, so the follow-up returns the draft, alone or with a command. Never show `isPending` booleans plus optional data.
