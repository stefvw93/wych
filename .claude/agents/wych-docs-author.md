...
- A feature is declared with `define({ props, state, action, output })` and built with `Feature.create({ initialState, reducer, render, subscriptions })`. `subscriptions` is optional. Props and state are `Schema.Struct`s. Actions and outputs are tagged vocabularies: `Action("Tag", fields)`, `Action.output("Tag", fields)`, `Action.of([...])`.
