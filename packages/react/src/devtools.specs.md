...
export interface DevtoolsEnvelope {
  readonly name: string; // from `component(bp, { name })`; `createFeatureStore` alone falls back to "WychFeature"
...
- [x] `name` comes from `component(bp, { name })`, which requires it; only a `createFeatureStore` driven by hand without a name falls back to `"WychFeature"`.
